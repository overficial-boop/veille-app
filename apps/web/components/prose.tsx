...
/** Shared markdown element renderers (safe subset, no raw HTML). Reused by the brief,
 *  which overrides `a` to render numbered citation superscripts. */
export const proseComponents: Components = {
  ...
  a: ({ node, ...p }) => <a className="cite" target="_blank" rel="noopener noreferrer" {...p} />,
...
export function Prose({ children, className }: { children: string; className?: string }) {
  return (
    <div className={className}>
      <ReactMarkdown components={proseComponents}>{children}</ReactMarkdown>
    </div>
  );
}
