...
export async function POST(req: NextRequest) {
  ...
  const body = (await req.json().catch(() => ({}))) as { intent?: string; plan?: unknown; autoBrief?: boolean };
  const intent = (body.intent ?? '').trim();
  if (!intent) return NextResponse.json({ error: 'intent required' }, { status: 400 });
  const autoBrief = body.autoBrief === true;
  try {
    const plan = await planDossier({ intent, language: 'fr', maxQueries: getRefreshConfig().plannerMaxQueries });
    const { slug } = await createDossier(session.user.id, intent, plan, autoBrief);
